 ...
 class Blockchain {
     ...
     _addBlock(block) {
         ...
         return new Promise(async (resolve, reject) => {
             try{const blockHeight = self.height + 1
    ...
    submitStar(address, message, signature, star) {
         ...
         return new Promise(async (resolve, reject) => {
                 ...
                 reject(Error("Block was not added: Timeout!"));
                }
         });
    }
     ...
     getStarsByWalletAddress (address) {
         ...
         return new Promise((resolve, reject) => {
             self.chain.forEach(async block =>{
                ...
                console.log(bdata)
                if (bdata.owner){
                    stars.push({"owner": bdata.owner, "star": bdata.star});
                }
             });
             resolve(stars)
                ...
                self.chain.forEach(async (block, index) =>{
                    ...
                    if (index > 0 ){
                        ...
                        if (block.previousBlockHash !== self.chain[index-1].hash){
                            errorLog.push(`${index}: Previous Block hash matching error`);
                        }
                    }
